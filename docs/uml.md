...
flowchart TD
    ...
    B --> C[担保の承認確認 (approve)]
    C --> D[担保の許可確認 (allowance)]
    D --> E[担保の転送 (transferFrom)]
    E --> F[引数のバリデーション]
    F --> G[ユーザーの状態確認 (isActive)]
    G --> H[ステーブルコインの発行]
